...
from core.config_dir.config import ElasticDep, env, get_elastic_settings
...
async def init_elasticsearch_index(index_name: str, db: Pool):
    async with db.acquire() as conn:
        "Вытягиваем из БД записи для индекса"
        conn = PgSql(conn)
        records = await conn.specialties.specialties2elastic()

    async with AsyncElasticsearch(**get_elastic_settings(env)) as aioes:
...
async def fast_search(body: AutocompleteSearchSchema, request: Request, es_client: ElasticDep):
    """
...
async def deep_search(body: DeepSearchSchema, pagen: PagenDep, es_client: ElasticDep):
    """
    код специальности и название склеить на фронте(возможно)
    """
    search_schema = search_ptn(body.search_term, search_mode=body.search_mode)
