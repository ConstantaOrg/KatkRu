...
async def get_all_versions(body: TtableVersionsGetSchema, pagen: PagenDep, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ttable_versions = await db.ttable.filtered_layout(
        request.state.building, body.status_id, body.type, body.is_commited,
...
async def search_by_id(body: PreAcceptTimetableSchema, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ttable_ver = await db.ttable.get_by_id(body.ttable_id, request.state.building)
    log_event(f'Нашли версию расписания | ttable_ver_id: \033[33m{ttable_ver.get("id") if ttable_ver else None}\033[0m; user_id: \033[31m{request.state.user_id}\033[0m; building_id: \033[35m{request.state.building}\033[0m', request=request)
...
async def replace_candidates(body: PreAcceptTimetableSchema, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ttable_candidates = await db.ttable.get_candidates_by_ver(body.ttable_id, request.state.building)
    log_event(f'Кандидаты для замены версии №\033[35m{body.ttable_id}\033[0m; length_candidates: \033[34m{len(ttable_candidates)}\033[0m ;user_id: \033[31m{request.state.user_id}\033[0m; building_id: \033[31m{request.state.building}\033[0m;', request=request)
