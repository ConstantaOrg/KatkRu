...
class N8NIUQueries:
    ...
    async def get_cards(self, sched_ver_id: int):
        query = '''
        SELECT csd.card_hist_id, csh.status_id AS status_card, csh.group_id, g.name, csd.position, csd.discipline_id, d.title, csd.week_day, csd.is_force
    ...
    async def load_std_lessons_as_current(self, building_id: int, week_day: int, sched_ver_id: int, user_id: int):
        ...
        std_recs AS (
            SELECT csh.group_id, csd.discipline_id, csd.position, csd.aud, csd.teacher_id
            FROM cards_states_details csd
            JOIN cards_states_history csh ON csh.id = csd.card_hist_id
            JOIN std_ver sv ON sv.id = csh.sched_ver_id
            JOIN groups g ON g.id = csh.group_id AND g.is_active = true
            JOIN teachers t ON t.id = csd.teacher_id AND t.is_active = true
            JOIN disciplines d ON d.id = csd.discipline_id AND d.is_active = true
            WHERE csh.status_id = $8
              AND csh.week_day = $4
        ),
        switch_cur AS (
        ...
        ins_hist AS (
            ...
            SELECT $5, $6, $7, s_r.group_id, true, NULL
            FROM std_recs s_r
            GROUP BY s_r.group_id
            RETURNING id, group_id
        ),
        ins_details AS (
            INSERT INTO cards_states_details (card_hist_id, discipline_id, "position", aud, teacher_id, sched_ver_id, week_day)
            SELECT h.id, s_r.discipline_id, s_r.position, s_r.aud, s_r.teacher_id, $5, NULL
            FROM std_recs s_r
            JOIN ins_hist h ON h.group_id = s_r.group_id
            RETURNING card_hist_id, position, discipline_id, week_day
        )
        SELECT i_d.card_hist_id, $7 AS status_card, g.id, g.name, i_d.position, d.id, d.title, i_d.week_day, false AS is_force
        ...
        '''
        res = await self.conn.fetch(query, TimetableVerStatuses.accepted, building_id, TimetableTypes.standard, week_day, sched_ver_id, user_id, CardsStatesStatuses.draft, CardsStatesStatuses.accepted)
        return res
    ...
    async def check_loaded_std_pairs(self, building_id: int, sched_ver_id: int):
        ...
        return {
            'diff_groups': [dict(row) for row in diff_groups],
            'diff_disciplines': [dict(row) for row in diff_disciplines],
            'diff_teachers': [dict(row) for row in diff_teachers]
        }


    async def get_ext_card(self, card_hist_id: int):
        query = '''
        SELECT csd.position, t.fio AS teacher_name, csd.teacher_id, csd.aud, csd.week_day FROM cards_states_details csd
        JOIN teachers t ON t.id = csd.teacher_id
    ...
    async def save_card(self, card_hist_id: int, sched_ver_id: int, user_id: int, week_day: int | None, lessons):
        query = '''
        WITH ver_check AS (
        ...
        ins_hist AS (
            INSERT INTO cards_states_history (sched_ver_id, user_id, status_id, group_id, is_current, week_day)
            SELECT $2, $3, $11, sc.group_id, true, $5
            FROM switch_cur sc
            RETURNING id AS new_hist_id
        ),
        ins_details AS (
            INSERT INTO cards_states_details (card_hist_id, discipline_id, "position", aud, teacher_id, is_force, sched_ver_id, week_day)
            SELECT 
                i_h.new_hist_id, UNNEST($6::int[]), UNNEST($7::int[]), UNNEST($8::text[]), UNNEST($9::int[]), UNNEST($10::bool[]), $2, UNNEST($12::smallint[])
            FROM ins_hist i_h
        )
        ...
        week_days = tuple(lesson.week_day for lesson in lessons)

        try:
            res = await self.conn.fetchval(
                query,
                card_hist_id, sched_ver_id, user_id, TimetableVerStatuses.accepted, week_day, 
                discipline_ids, positions, auds, teacher_ids, is_forces, CardsStatesStatuses.edited, week_days
            )
    ...
    async def get_cards_history(self, sched_ver_id: int, group_id: int):
        query = '''
        SELECT csh.id AS card_hist_id, csh.created_at, csh.user_id, u.name AS user_name, csh.status_id, csh.is_current, csh.week_day
    ...
    async def get_card_content(self, card_hist_id: int):
        query = '''
        SELECT csd.position, csd.aud, csd.discipline_id, d.title AS discipline_title, csd.teacher_id, t.fio AS teacher_name, csd.week_day, csd.is_force
        FROM cards_states_details csd
        JOIN disciplines d ON d.id = csd.discipline_id
        JOIN teachers t ON t.id = csd.teacher_id
        WHERE csd.card_hist_id = $1
    ...
    async def bulk_add_cards(self, ttable_id: int, user_id: int, week_day: int | None, group_names: list[str], lessons: list):
        """
        ...
        if lessons:
            ...
            week_days = [lesson.week_day for lesson in lessons]
        else:
            discipline_ids = []
            positions = []
            auds = []
        ...
        ins_hist AS (
            INSERT INTO cards_states_history (sched_ver_id, user_id, status_id, group_id, is_current, week_day)
            SELECT $1, $3, $4, g.id, true, $5
            FROM group_ids g
            WHERE EXISTS (
                SELECT 1 FROM ver_check 
                WHERE is_commited = false OR status_id != $6
            )
            RETURNING id, group_id
        ),
        ins_details AS (
            ...
            SELECT 
                ...
                lesson_data.week_day
            FROM ins_hist h
            CROSS JOIN (
                SELECT 
                    UNNEST($7::int[]) as discipline_id,
                    UNNEST($8::int[]) as position,
                    UNNEST($9::text[]) as aud,
                    UNNEST($10::int[]) as teacher_id,
                    UNNEST($11::bool[]) as is_force,
                    UNNEST($13::smallint[]) as week_day
            ) lesson_data
            WHERE $12 > 0
        ...
        res = await self.conn.fetchrow(
            query,
            ttable_id, group_names, user_id, CardsStatesStatuses.draft, week_day, TimetableVerStatuses.accepted,
            discipline_ids, positions, auds, teacher_ids, is_forces, len(discipline_ids), week_days
        )
